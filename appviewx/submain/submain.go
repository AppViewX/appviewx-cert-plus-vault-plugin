...
import (
	...
	"errors"
	"fmt"
	"hashicorp_vault_plugin/appviewx/appConnector"
	"hashicorp_vault_plugin/appviewx/ca"
	"hashicorp_vault_plugin/appviewx/certSettings"
	"hashicorp_vault_plugin/appviewx/certificate"
	"hashicorp_vault_plugin/appviewx/certificateChain"
	"hashicorp_vault_plugin/appviewx/certificateDownload"
	"hashicorp_vault_plugin/appviewx/common"
	"hashicorp_vault_plugin/appviewx/config"
	"hashicorp_vault_plugin/appviewx/csr"
	"hashicorp_vault_plugin/appviewx/session"
	"hashicorp_vault_plugin/keygenerator"
	"log"
	"net"
)
...
func handleCertificateGeneration(csrContent []byte, currentCert *certificate.CurrentCert, config *config.Config, ca *ca.CA, certSettings *certSettings.CertSettings) (certificateID string, err error) {
	...
	csrContentString := string(csrContent)

	csrResponse, err := uploadCSR(&csrContentString, currentCert, config, ca, certSettings, sessionID)
	if err != nil {
	...
	certResponse, err := createCertificate(&csrContentString, csrResponse, currentCert, config, ca, certSettings, sessionID)
	if err != nil {
...
func DownloadCertificateForCertificateID(certificateID string, config *config.Config) (isAllAvailable bool, finalSerialNumber string, certificateOutput []byte, uuid string, err error) {
	...
	log.Println("Default isAllAvailable true set")
	isAllAvailable = true
	...
	if err != nil {
		...
		log.Println("isAllAvailable set to false")
		return
	}
...
func createCertificate(csrContent *string, csrResponse csr.Response, currentCert *certificate.CurrentCert, config *config.Config, ca *ca.CA, certSettings *certSettings.CertSettings, sessionID string) (output certificate.Response, err error) {

	payload := certificate.GeneratePayloadForCertificateCreate(csrContent, currentCert, ca, certSettings)
